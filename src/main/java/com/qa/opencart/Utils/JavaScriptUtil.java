...
public class JavaScriptUtil {
	...
	public void generateAlertByJs(String mesg) {
		js.executeAsyncScript("alert('"+mesg+"')");
		try{
			Thread.sleep(2000);
		}catch(Exception e) {
			e.printStackTrace();
	...
	public void generateConfirmByJs(String mesg) {
		js.executeAsyncScript("confirm('"+mesg+"')");
		try{
			Thread.sleep(2000);
		}catch(Exception e) {
			e.printStackTrace();
	...
	public void generateJsPrompt(String mesg, String value) {
		js.executeAsyncScript("prompt('"+mesg+"')");
		try{
			Thread.sleep(2000);
		}catch(Exception e) {
			e.printStackTrace();
	...
	public void goBackWithJs() {
		js.executeAsyncScript("history.go(-1)");
	}
	...
	public void ScriptText(String text) {
		String confirmPasswordScript = "return window.getComputedStyle(document.querySelector(\"label[for='"+text+"']\"),'::before').getPropertyValue('content');";
		String confirmPasswordText = js.executeScript(confirmPasswordScript).toString();
		System.out.println(confirmPasswordText);
		if(confirmPasswordText.contains("*")) {
			System.out.println("text is mandatory");
		}
	}
